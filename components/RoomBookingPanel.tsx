...
/** Returns all dates (YYYY-MM-DD) that fall within a booked range (inclusive both ends) */
function getBlockedDates(ranges: BookedRange[]): Set<string> {
...
/** Checks if a date range overlaps any booked range */
function rangeOverlapsBooking(
  ...
  for (const range of bookedRanges) {
    ...
    if (start < bookedEnd && end > bookedStart) return true;
  }
  return false;
}
...
export default function RoomBookingPanel({ room }: RoomBookingPanelProps) {
  ...
  const blockedDates = useMemo(() => getBlockedDates(bookedRanges), [bookedRanges]);
  ...
  const isOverlapping = useMemo(
    () => rangeOverlapsBooking(checkIn, checkOut, bookedRanges),
  ...
  return (
    <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-200/80 flex flex-col gap-6 sticky top-24">
      ...
      {!loadingDates && bookedRanges.length > 0 && (
        <div className="bg-amber-50 border border-amber-200/60 rounded-xl px-4 py-3 text-xs text-amber-800 flex flex-col gap-2">
          <p className="font-semibold flex items-center gap-1.5">
            <AlertCircle className="h-3.5 w-3.5 shrink-0" />
            Some dates are already reserved
          </p>
          <ul className="flex flex-col gap-1 text-amber-700">
      ...
      <div className="flex flex-col gap-4">
        ...
        {isOverlapping && (
          <div className="flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-xl px-4 py-2.5 text-xs text-rose-700 font-medium">
            <AlertCircle className="h-3.5 w-3.5 shrink-0" />
            <span>These dates overlap an existing reservation. Please choose different dates.</span>
          </div>
        )}
      ...
      <div className="bg-stone-50 p-4 rounded-2xl border border-slate-200/80 flex flex-col gap-2.5">
        <div className="flex justify-between text-sm">
          <span className="text-slate-500">${pricePerNight} x {nights} {nights === 1 ? "night" : "nights"}</span>
